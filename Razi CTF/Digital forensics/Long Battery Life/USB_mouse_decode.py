...
def main():
    ...
    if len(sys.argv) != 3:
        print("Usage : ")
        print("        python2 USB_mouse_decode.py data.pcap [LEFT|RIGHT|MOVE|ALL]")
        print("Tips : ")
        print("        To use this python script , you must install the numpy,matplotlib and tkinter first.")
        print("        You can use `sudo pip install matplotlib numpy` to install it")
        print("        You can use `sudo apt install python-tk` to install it")
        print("Author : ")
        print("        >>> Ignite <arijeet.pappu@gmail.com>")
        exit(1)
    ...
    # handle move
    for i in data:
        Bytes = [(i[j:j+2]) for j in range(0, len(i), 2)]
        if len(Bytes) == 8:
            horizontal = 2  # -
            vertical = 4  # |
        elif len(Bytes) == 4:
    ...
    ax1.set_title('[%s]-[%s]' % (pcapFilePath, action))
